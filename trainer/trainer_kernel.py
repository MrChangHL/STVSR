...
class Trainer_Kernel(Trainer):
    ...
    def conv_func(self, input, kernel, padding='same'):  # 实现了一个针对输入张量的每个通道进行卷积操作的函数，并返回卷积结果
        ...
        conv_result_list = []
        for i in range(c):
            conv_result_list.append(F.conv2d(input[:, i:i + 1, :, :], kernel, bias=None, stride=1, padding=pad))   # 使用F.conv2d函数对每个通道进行卷积操作
    ...
    def blur_down(self, x, kernel, scale):  # 模糊和下采样的函数。首先，它将输入张量 x 进行模糊操作，然后对模糊结果进行下采样
        ...
        psize = ksize // 2               # 用于确定填充 (padding) 的大小
        assert kc == 1, "only support kc=1!" # 确保卷积核的通道数 kc 为 1，也就是说只支持单通道的卷积核。

        # blur
        x = F.pad(x, (psize, psize, psize, psize), mode='replicate')  # 对输入张量 x 进行填充操作
        blur_list = []
        for i in range(b):
            blur_list.append(self.conv_func(x[i:i + 1, :, :, :], kernel[i:i + 1, :, :, :]))  # 按批次对输入张量x卷积操作
        blur = torch.cat(blur_list, dim=0)
        blur = blur[:, :, psize:-psize, psize:-psize]  # 去除填充

        # down
        blurdown = blur[:, :, ::scale, ::scale]  # 沿着高度和宽度维度每隔 scale (采样尺度)个元素进行取样，实现下采样
