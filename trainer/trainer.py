...
class Trainer:
    def __init__(self, args, loader, my_model, my_loss, ckp):
        ...
        self.model = my_model
        self.loss = my_loss
        self.optimizer = self.make_optimizer()        # 优化器
        self.scheduler = self.make_scheduler()        # 等间隔调整学习率
        self.ckp = ckp

        if args.load != '.':    # 判断是否设置了加载预训练模型的路径
    ...
    def terminate(self):   # 确定是否终止训练过程
        if self.args.test_only:
            self.test()
            return True
        else:
            epoch = self.scheduler.last_epoch + 1
            return epoch >= self.args.epochs    # 判断是否达到了指定的训练周期数
