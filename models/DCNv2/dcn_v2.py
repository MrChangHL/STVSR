...
class DCN(DCNv2):       # 添加了一个额外的卷积层 conv_offset_mask，用于生成偏移量和掩码
    def __init__(
        self,
        in_channels,
        out_channels,
    ...
    def forward(self, input):
        out = self.conv_offset_mask(input)       # 生成偏移量和掩码
...
class DCN_sep(DCNv2):         # 接收两个输入特征 input 和 fea，其中 input 是用于可变形卷积的输入特征，而 fea 是用于生成偏移量和掩码的其他特征
