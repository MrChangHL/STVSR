...
from models.DenoisingNAFNet_arch import *
from models.TTST_arc import MSF
# from modules.CSVSR import NLMaskBranchDownUp, default_conv
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        #     RCAB, num_basic_block=1, num_feat=hidden_dim, squeeze_factor=16, res_scale=1)
        Extra_feat = []
        Extra_feat.extend([GCA(dim=hidden_dim)
                           for _ in range(5)])
        self.extra_feat = nn.Sequential(*Extra_feat)
        # self.conv = nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 1)
        mlp_hidden_dim = int(hidden_dim * 2.)
        self.mlp = Mlp(in_features=hidden_dim, hidden_features=mlp_hidden_dim, act_layer=nn.GELU, drop=0.)
        # self.GCA = GCA(dim=hidden_dim)
        # self.nmffm = NMFFM(num_feat=hidden_dim, num_frame=5, center_frame_idx=2)
        # self.NLB = NLMaskBranchDownUp(default_conv, hidden_dim, 3)

        # self.hybridgate = HybridGate(dim=hidden_dim, mlp_ratio=2.)

    def forward(self, input, x_size):
        B, L, C = input.shape
        input1 = input.view(B, *x_size, C).contiguous()  # [B,H,W,C]
        input2 = input.view(B, *x_size, C).permute(0, 3, 1, 2).contiguous()  # [B,C,H,W]
        x = self.ln_1(input1)
        x = input1*self.skip_scale + self.drop_path(self.self_attention(x)) + self.drop_path(self.extra_feat(input2).permute(0, 2, 3, 1).contiguous())
        # x = x*self.skip_scale2 + self.MFL(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()

        # x = x * self.skip_scale2 + self.conv_blk(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous()
        x = x*self.skip_scale2 + self.drop_path(self.mlp(self.ln_2(x).contiguous()).contiguous())
