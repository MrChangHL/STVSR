...
class multiheadAttention(nn.Module):
    ...
    def __init__(self, dim=180, num_heads=6, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
                 position_bias=True):
...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y1, y2, y3, y4 = self.forward_core(x)     # 2D SSM
...
class FreMamba(nn.Module):
    ...
    def forward_features(self, x):   # [4,96,64,64]
        x_size = (x.shape[2], x.shape[3])
        x = self.patch_embed(x)      # N,L,C

        x = self.pos_drop(x)         # N,L,C
...
if __name__ == '__main__':
    ...
    input = torch.rand(1, 3, size, size).cuda()  # 输入 B C H W
    model = FreMamba().cuda()
    out = model(input)
