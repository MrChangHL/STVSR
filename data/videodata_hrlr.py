...
class VIDEODATA_HRLR(data.Dataset):
    ...
    def _load_file(self, idx):
        ...
        gts = np.array([imageio.v2.imread(hr_name) for hr_name in f_gts], dtype=np.float)
        inputs = np.array([imageio.v2.imread(lr_name) for lr_name in f_inputs], dtype=np.float)
