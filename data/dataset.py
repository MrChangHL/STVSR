...
def load_img_future(filepath, nFrames, scale, other_dataset):
    ...
    if other_dataset:
        ...
        for i in seqLQs:
            input.append(modcrop(Image.open(filepath+'/im'+str(i)+'.png').convert('RGB'), scale).resize((int(target[0].size[0]/scale),int(target[0].size[1]/scale)),Image.BICUBIC))
...
def save_flow_as_image(flow, filename):
    # Normalize the flow values for visualization
...
if __name__ == '__main__':
    folder_path = '/home/chl/桌面/exp_result/0'  # 替换为你的文件夹路径

    # 读取文件夹中的所有PNG图像
    input_images = read_images_from_folder(folder_path)

    # 对相邻的图像进行光流计算并保存
    for i in range(len(input_images) - 1):
        # 计算正向光流（im1 -> im2）
        f_flow = get_flow(input_images[i], input_images[i + 1])
        # 计算反向光流（im2 -> im1）
        b_flow = get_flow(input_images[i + 1], input_images[i])

        # 保存光流图
        save_flow_as_image(f_flow, f'f_flow_{i}.png')
        save_flow_as_image(b_flow, f'b_flow_{i}.png')
